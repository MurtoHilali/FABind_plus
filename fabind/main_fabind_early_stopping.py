...
def prepare_model_and_optimizer(args, accelerator, logger):
    ...
    if args.optim == "adam":
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, foreach=True)
    elif args.optim == "adamw":
        optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, foreach=True)
    elif args.optim == "sgd":
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, foreach=True)
...
def main():
    ...
    best_validation_loss = float('inf')  # Use 'inf' for loss or '-inf' for accuracy
    patience = args.early_stopping_patience  # Set patience as a command-line argument or config
    ...
    for epoch in range(last_epoch+1, args.total_epochs):
        ...
        if epoch % args.test_interval == 0:
            ...
            if accelerator.is_main_process:
                ...
                val_metrics = validate(accelerator, args, valid_loader, accelerator.unwrap_model(model), com_coord_criterion, distmap_criterion, pocket_cls_criterion, pocket_coord_criterion, pocket_radius_criterion, accelerator.device, stage=2)
                logger.log_stats(val_metrics, epoch, args, prefix="Valid_pp")
                if args.wandb:
                    log_to_wandb(val_metrics, "valid_pp", epoch)


                ## Early stopping logic
                current_validation_loss = val_metrics['com_coord_huber_loss'] + val_metrics['contact_loss'] + val_metrics['contact_by_pred_loss'] + val_metrics['pocket_cls_bce_loss'] + val_metrics['pocket_radius_pred_loss'] + val_metrics['pocket_coord_mse_loss']
                epsilon = 0.1
                if current_validation_loss < best_validation_loss - epsilon:
                    best_validation_loss = current_validation_loss
                    epochs_without_improvement = 0
                    logger.log_message(f"Validation log improved to {best_validation_loss:.4f}")
                    best_model_dir  = f"{root_path}/models/best_model"
                    accelerator.save_state(output_dir=best_model_dir, safe_serialization=False)
                else:
                    epochs_without_improvement += 1
                    logger.log_message(f"No improvement in validation metric for {epochs_without_improvement} epochs.")

                if epochs_without_improvement >= patience:
                    logger.log_message(f"Early stopping triggered after {epoch} epochs.")
                    break
